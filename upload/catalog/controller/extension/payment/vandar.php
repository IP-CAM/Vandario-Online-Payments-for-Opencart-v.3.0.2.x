...
/* **********************Update to Rest payment vandar Alireza Yousefi ***************************** */
...
class ControllerExtensionPaymentvandar extends Controller {
    public function index() {
        ...
        return $this->load->view('extension/payment/vandar', $data1);
    }
    ...
    public function confirm() {
        ...
        $MerchantID = $this->config->get('payment_vandar_pin');  	//Required
        ...
        $Email = isset($order_info['order_id']) ? $order_info['order_id'] : ''; 	// Optional
        ...
        $CallbackURL = $this->url->link('extension/payment/vandar/callback', 'order_id=' . $data1['order_id'], true);  // Required

        $data = array('api_key' => $MerchantID,
            'amount' => $Amount,
            'factorNumber' 		=> $Email,
        ...
        $ch = curl_init('https://vandar.io/api/ipg/send');
        curl_setopt($ch, CURLOPT_USERAGENT, 'vandar Rest Api v1');
        ...
        if(!$result) {
            ...
            //$data1['action'] = "https://vandar.io/ipg/" . $result->token;
            $data1['action'] = "https://vandar.io/ipg/" . $result->token;
    ...
    public function callback() {
        if ($this->session->data['payment_method']['code'] == 'vandar') {
            $this->load->language('extension/payment/vandar');
            ...
            $data1['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('extension/payment/vandar/callback', '', true)
            );
            ...
            try {
                ...
                switch ( array_keys($verifyResult)[0] ) {
                    case 'RefID': // success
                        ...
                        $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_vandar_order_status_id'), $comment, true);
            ...
            $this->response->setOutput($this->load->view('extension/payment/vandar_confirm', $data1));
        }
    }
    ...
    private function verifyPayment($authority, $amount) {
        $MerchantID = $this->config->get('payment_vandar_pin');

        $data = array('api_key' => $MerchantID, 'token' => $authority);
        $jsonData = json_encode($data);
        $ch = curl_init('https://vandar.io/api/ipg/verify');
        curl_setopt($ch, CURLOPT_USERAGENT, 'vandar Rest Api v1');
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
        curl_setopt($ch, CURLOPT_POSTFIELDS, $jsonData);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
