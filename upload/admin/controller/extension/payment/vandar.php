...
class ControllerExtensionPaymentvandar extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/vandar', 'user_token=' . $this->session->data['user_token'], true)
		);

		$data['action'] = $this->url->link('extension/payment/vandar', 'user_token=' . $this->session->data['user_token'], true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=payment', true);

		if (isset($this->request->post['payment_vandar_pin'])) {
			$data['payment_vandar_pin'] = $this->request->post['payment_vandar_pin'];
		} else {
			$data['payment_vandar_pin'] = $this->config->get('payment_vandar_pin');
		}

		if (isset($this->request->post['payment_vandar_host_type'])) {
			$data['payment_vandar_host_type'] = $this->request->post['payment_vandar_host_type'];
		} else {
			$data['payment_vandar_host_type'] = $this->config->get('payment_vandar_host_type');
		}
	...
	protected function validate() {
		...
		if (!$this->request->post['payment_vandar_pin']) {
